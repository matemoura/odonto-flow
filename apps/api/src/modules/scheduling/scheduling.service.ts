...
export class SchedulingService {
   ...
   * embaixo dela.
   */
  private async getConfigDaClinica(clinicId: string): Promise<{ timezone: string; dias: number[] }> {
    const clinic = await this.prisma.clinic.findUniqueOrThrow({
      where: { id: clinicId },
      select: { timezone: true, workingWeekdays: true },
    });
    return {
  ...
  /** Dias de atendimento da clínica, para a tela de Organização. */
  async getSchedulingSettings(clinicId: string) {
    const { dias } = await this.getConfigDaClinica(clinicId);
    return { workingWeekdays: dias };
  }

  async updateSchedulingSettings(clinicId: string, workingWeekdays: number[]) {
    // Ordena e tira repetido: o valor chega de checkbox e a ordem da marcação
    // não deveria virar a ordem guardada no banco.
    const dias = [...new Set(workingWeekdays)].sort((a, b) => a - b);
    const clinic = await this.prisma.clinic.update({
      where: { id: clinicId },
      data: { workingWeekdays: dias },
      select: { workingWeekdays: true },
    });
    return { workingWeekdays: clinic.workingWeekdays };
  }
  ...
  async getBookableDays(clinicId: string, professionalId: string, count = 6, fromIso?: string) {
    const { timezone, dias } = await this.getConfigDaClinica(clinicId);
