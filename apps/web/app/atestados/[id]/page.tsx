...
export const metadata = { title: "Atestado — Odonto Flow" };

export default async function ImprimirAtestadoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let certificate;
  try {
    certificate = await getCertificate(session.clinicSlug, session.token, id);
  } catch (error) {
    if (error instanceof ApiError && error.status === 404) notFound();
    return (
      <div style={{ padding: 40, fontSize: 13.5 }}>
        <p>Não foi possível carregar este atestado agora.</p>
      </div>
    );
  }
